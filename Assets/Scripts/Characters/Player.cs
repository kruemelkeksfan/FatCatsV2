...
public class Player : MonoBehaviour
{
	...
	[SerializeField] private int playerFiguresPerLevel = 18;
	[SerializeField] private float playerFigureHeight = 0.2f;
	[SerializeField] private float playerFigureDiameter = 0.06f;
	...
	public void SetPosition(Tile tile, Map newMap = null)
	{
		if(newMap != null)
		{
		...
		// Calculate Player Figure Position on Tile
		int playerPositionIndex = tile.ReservePlayerPosition(this);
		// First, we calculate at which Level we will place the Player Figure:
		int playerFigureLevel = playerPositionIndex / playerFiguresPerLevel;
		playerPositionIndex %= playerFiguresPerLevel;
		// How to get from an Hex Position Index to the Ring Index?
		// Our Hexagon Indices look like this:
		//      7 18 17
		//    8  1  6 16
		//  9  2  0  5 15
		//   10  3  4 14
		//     11 12 13
		// Each Ring (except the First) has (Ring Index * 6) Positions.
		// So each Ring consists of Ring Segments of 6 Positions each.
		// We will first determine the Segment of our Player Position Index with Integer Division:
		int segmentIndex = (playerPositionIndex > 0) ? (((playerPositionIndex - 1) / 6) + 1) : 0;
		// Since each Ring Level gains one Segment more, we can use Triangular-Number-Magic to determine which Ring Level/Index belongs to each Segment.
		// Source: https://de.wikipedia.org/wiki/Dreieckszahl#Dreieckswurzel
		int ringIndex = (segmentIndex > 0) ? Mathf.CeilToInt((Mathf.Sqrt(8 * segmentIndex + 1) - 1) * 0.5f) : 0;
		int positionOnRing = playerPositionIndex - Mathf.Max(((ringIndex - 1) * 6 + 1), 0); // Edge Case: Ring 0
		int totalPositionsOnRing = (ringIndex > 0) ? (ringIndex * 6) : 1;
		float positionAngle = ((float) positionOnRing / (float) totalPositionsOnRing) * Mathf.PI * 2.0f;
		Vector3 playerPosition = new Vector3(-Mathf.Cos(positionAngle) * ringIndex * playerFigureDiameter, playerFigureLevel * playerFigureHeight, Mathf.Sin(positionAngle) * ringIndex * playerFigureDiameter);

		transform.SetParent(tile.GetPlayerParent(), false);
		transform.SetLocalPositionAndRotation(playerPosition, Quaternion.identity);
		if(isLocal)
		{
